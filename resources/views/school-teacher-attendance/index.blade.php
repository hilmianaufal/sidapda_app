...
@section('title', 'Absensi Guru '.$institution->short_name)
@section('mobile_title', 'Absensi Guru')

@section('content')
<x-ui.page-header
  :title="'Absensi Guru '.$institution->short_name"
  :subtitle="'Scan masuk dan pulang • Tahun ajaran '.$academicYear"
  icon="bi-person-check"
>
  <x-slot:actions>
    <x-ui.button :href="route('school-teacher-attendance.excuses.index', $institution)">
...
<div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
  ...
  <x-ui.stat-card label="Sudah Pulang" :value="$checkedOut" icon="bi-box-arrow-right" tone="blue" />
  <x-ui.stat-card label="Pulang Cepat" :value="$earlyLeave" icon="bi-exclamation-triangle" tone="red" />
</div>
  ...
  <div class="lg:col-span-7">
    <x-ui.card>
      ...
      <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div class="text-lg font-black text-slate-900">Scanner QR/Kode Guru</div>
      ...
      <div id="manualWrap" class="hidden">
        <div class="rounded-[2rem] border border-slate-100 bg-slate-50 p-6">
          <div class="font-black text-slate-900">Input Manual QR/Kode Guru</div>
          <div class="mt-4 flex gap-3">
            <input id="manualToken" class="min-w-0 flex-1 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold outline-none focus:ring-4 focus:ring-emerald-100" placeholder="Token QR atau Kode Guru" autocomplete="off">
  ...
  <div class="lg:col-span-5">
    <x-ui.card>
      <div class="mb-5">
        ...
        <div class="text-sm font-medium text-slate-500">Identitas dan status guru tampil di sini.</div>
      </div>
      ...
      <div id="resultBox" class="rounded-[2rem] border border-dashed border-slate-200 bg-slate-50 p-8 text-center">
        ...
        <div class="mt-1 text-sm text-slate-400">Pilih jenis absensi lalu scan QR guru.</div>
      </div>
    ...
    <div class="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-700">
      Pulang cepat: <strong>{{ $earlyLeave }}</strong> • Izin: <strong>{{ $excused }}</strong> • Sakit: <strong>{{ $sick }}</strong> • Lainnya: <strong>{{ $otherExcuses }}</strong>. Data diperbarui setelah halaman dimuat ulang.
    </div>
  </div>
</div>
        ...
        <tbody>
          @forelse($todayAttendances as $attendance)
            @php
              ...
              $levelLabel = match($attendance->level_snapshot) {
                'mi' => 'MI',
                'mts' => 'MTs',
                'ma' => 'MA',
                'mts_ma' => 'MTs & MA',
            ...
            <tr class="border-b border-slate-100">
              <td class="px-3 py-3">
                <div class="font-black text-slate-800">{{ $attendance->teacher?->name ?? 'Guru dihapus' }}</div>
                <div class="text-xs font-semibold text-slate-400">Kode Guru {{ $attendance->teacher?->teacher_code ?? $attendance->teacher_code_snapshot }}</div>
              </td>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function sendToken(rawToken) {
    ...
    try {
      ...
      if (!response.ok || !json.ok) {
        const message = json.message ?? 'QR/Kode Guru tidak valid.';
      ...
      playBeep();
      speak(`${json.teacher.name}, ${json.status_label}`);
      if (navigator.vibrate) navigator.vibrate(80);

      resultBox.innerHTML = `
        <div class="rounded-[2rem] bg-gradient-to-br ${tone} p-5 text-white shadow-xl">
          <div class="flex items-start gap-4 text-left">
            <img src="${escapeHtml(photo)}" class="h-20 w-20 rounded-3xl object-cover ring-4 ring-white/70 shadow-xl" alt="Foto guru">
            <div class="min-w-0 flex-1">
              <div class="text-xl font-black leading-tight">${escapeHtml(json.teacher.name)}</div>
              <div class="mt-1 text-sm font-bold text-white/80">Kode Guru: ${escapeHtml(json.teacher.teacher_code)}</div>
              <div class="mt-3 flex flex-wrap gap-2">
                <span class="rounded-full bg-white/20 px-3 py-1 text-xs font-black">${escapeHtml(json.teacher.level ?? '-')}</span>
              </div>
            </div>
          </div>
  ...
  async function startScan() {
    ...
    try {
      ...
      showAlert('info', 'Kamera aktif. Arahkan ke QR guru.');
    } catch (error) {
      isScanning = false;
  ...
  btnManual.addEventListener('click', () => {
    ...
    if (!token) {
      showAlert('warning', 'QR/Kode Guru masih kosong.');
      return;
    }
    manualToken.value = '';
    sendToken(token);
