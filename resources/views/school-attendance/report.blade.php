...
@section('title', 'Rekap Absensi Siswa '.$institution->short_name)
@section('mobile_title', 'Rekap '.$institution->short_name)

@section('content')
<x-ui.page-header
  :title="'Rekap Absensi Siswa '.$institution->short_name"
...
>
  <x-slot:actions>
    ...
    <x-ui.button :href="route('school-attendance.excuses.index', ['institution' => $institution, 'date' => $date])" variant="secondary">
...
<x-ui.card class="mb-6">
  <div class="mb-4">
    ...
    <div class="text-sm font-medium text-slate-500">Pilih tanggal, jenjang, dan kelas.</div>
  </div>

  <form method="GET" action="{{ route('school-attendance.reports.index', $institution) }}" class="grid gap-3 md:grid-cols-5">
    <div>
    ...
    <div>
      ...
      <select id="class_name" name="class_name" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold outline-none focus:ring-4 focus:ring-emerald-100">
        ...
        @foreach($classOptions as $classOption)
          <option value="{{ $classOption }}" @selected($className === $classOption)>{{ $classOption }}</option>
        @endforeach
      </select>
    </div>
    ...
    <div class="flex items-end gap-2">
      ...
      <a href="{{ route('school-attendance.reports.index', $institution) }}" class="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-700">
        Reset
      </a>
    </div>
    ...
    <div class="flex items-end">
      <a
        href="{{ route('school-attendance.reports.export', array_filter([
...
<div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
  <x-ui.stat-card label="Total Siswa" :value="$summary['total']" icon="bi-people" tone="slate" />
      ...
      <tbody>
        @forelse($rows as $row)
          ...
          <tr class="border-b border-slate-100 align-top">
            ...
            <td class="px-3 py-3 font-bold text-slate-600">{{ $row['level'] }} / {{ $row['class_name'] }}</td>
            <td class="px-3 py-3">
            ...
            <td class="max-w-xs px-3 py-3 font-semibold text-slate-600">
              ...
              @if($row['has_attachment'])
                <a
                  href="{{ route('school-attendance.reports.excuses.download', [$institution, $row['excuse_id']]) }}"
        ...
        @empty
          <tr>
            <td colspan="7" class="px-4 py-10 text-center font-bold text-slate-400">
              Belum ada siswa aktif pada filter ini.
            </td>
          </tr>
        @endforelse
      </tbody>
