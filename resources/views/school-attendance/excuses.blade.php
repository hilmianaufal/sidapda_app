...
@section('content')
<x-ui.page-header
  :title="'Izin & Sakit Siswa '.$institution->short_name"
  subtitle="Diisi guru piket dan dilengkapi surat"
  icon="bi-file-earmark-medical"
>
  <x-slot:actions>
    <x-ui.button :href="route('school-attendance.index', $institution)" variant="secondary">
  ...
  <div class="lg:col-span-5">
    <x-ui.card>
      <div class="mb-5">
        ...
        <div class="text-sm font-medium text-slate-500">Satu status untuk setiap siswa per tanggal.</div>
      </div>

      @if($studentOptions->isEmpty())
        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold text-amber-700">
          Belum ada siswa aktif di {{ $institution->short_name }} tahun ajaran {{ $academicYear }}.
        </div>
      @else
        <form method="POST" action="{{ route('school-attendance.excuses.store', $institution) }}" enctype="multipart/form-data" class="space-y-4">
          @csrf
  ...
  <div class="lg:col-span-7">
    <x-ui.card>
      <div class="mb-4">
        ...
        <div class="text-sm font-medium text-slate-500">Cari berdasarkan tanggal, status, kelas, atau siswa.</div>
      </div>

      <form method="GET" action="{{ route('school-attendance.excuses.index', $institution) }}" class="grid gap-3 sm:grid-cols-2">
        <input
          type="date"
          name="date"
          value="{{ $date }}"
        ...
        <div class="flex gap-3 sm:col-span-2">
          ...
          <a href="{{ route('school-attendance.excuses.index', $institution) }}" class="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-700">
            Reset
          </a>
        </div>
      </form>
          ...
          @forelse($excuses as $excuse)
            ...
            <tr class="border-b border-slate-100 align-top">
              <td class="px-3 py-3">
                ...
                <div class="text-xs font-semibold text-slate-400">NIS {{ $excuse->student?->nis ?? '-' }}</div>
              </td>
              <td class="px-3 py-3 font-bold text-slate-600">
                {{ $excuse->level_snapshot ?: '-' }} / {{ $excuse->class_name_snapshot ?: '-' }}
              </td>
              <td class="px-3 py-3">
              ...
              <td class="px-3 py-3">
                @if($excuse->attachment_path)
                  <a href="{{ route('school-attendance.excuses.download', [$institution, $excuse]) }}" class="inline-flex items-center gap-2 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700">
              ...
              <td class="px-3 py-3 text-right">
                <form method="POST" action="{{ route('school-attendance.excuses.destroy', [$institution, $excuse]) }}" onsubmit="return confirm('Hapus status dan lampiran ini?')">
                  @csrf
                  @method('DELETE')
